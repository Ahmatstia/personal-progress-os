...
export async function GET(request: Request) {
  ...
  catch (error) { if (error instanceof AuthorizationError) return authErrorResponse(error); return NextResponse.json({ success: false, error: { message: "Analytics gagal dimuat.", code: "INTERNAL_ERROR" } }, { status: 500 }); }
}
