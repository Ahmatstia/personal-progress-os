...
export async function PATCH(request: Request, context: Context) {
  ...
  try { const user = await requireCurrentUser(); return NextResponse.json({ success: true, data: await reorderTodayFocus((await context.params).id, parsed.data.direction, user.id) }); } catch (error) { return error instanceof Error && error.message === "Autentikasi diperlukan." ? authErrorResponse(error) : errorResponse(error); }
}
