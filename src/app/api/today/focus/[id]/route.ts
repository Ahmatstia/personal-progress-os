...
export async function PATCH(request: Request, context: Context) {
  ...
  try { const user = await requireCurrentUser(request); return NextResponse.json({ success: true, data: await reorderTodayFocus((await context.params).id, parsed.data.direction, user.id) }); } catch (error) { return error instanceof AuthorizationError ? authErrorResponse(error) : errorResponse(error); }
}
