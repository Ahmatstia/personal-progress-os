...
export async function DELETE(_request: Request, context: Context) { try { await requireCurrentUser(); return NextResponse.json({ success: true, data: await removeTodayFocus((await context.params).id) }); } catch (error) { return error instanceof Error && error.message === "Autentikasi diperlukan." ? authErrorResponse(error) : errorResponse(error); } }
export async function PATCH(request: Request, context: Context) {
  ...
  try { await requireCurrentUser(); return NextResponse.json({ success: true, data: await reorderTodayFocus((await context.params).id, parsed.data.direction) }); } catch (error) { return error instanceof Error && error.message === "Autentikasi diperlukan." ? authErrorResponse(error) : errorResponse(error); }
}
