...
export async function POST(request: Request) {
  let user;
  try { user = await requireCurrentUser(); } catch (error) { return authErrorResponse(error); }
  let body: unknown;
  ...
  try {
    const result = process.env.NODE_ENV === "test" ? await executeAICommand(parsed.data) : await executeAICommand(parsed.data, user.id);
