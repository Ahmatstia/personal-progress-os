...
export async function getPeriodAnalytics(start: Date, end: Date, goalId?: string) { return buildAnalytics(await findAnalyticsGoals(goalId) as unknown as Goal[], start, end); }
export async function getDashboardAnalytics(query: AnalyticsQuery = { days: 30 }) {
  const end = query.end ?? new Date();
  const start = query.start ?? new Date(startOfDay(end).getTime() - ((query.days - 1) * 86400000));
  return getPeriodAnalytics(start, end, query.goalId);
}
