...
The corpus is intentionally phrase-oriented. Scores are a baseline, not evidence of production language understanding. Use `--json` for the complete error analysis report. Phase 10 compares v1 and v2 with the same pipeline; v2 remains an evaluation corpus and does not replace the Phase 8 production classifier.
