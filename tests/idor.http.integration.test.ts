...
describe("Phase 15 HTTP-level two-user security", () => {
  beforeAll(async () => {
    ...
    goalA = await prisma.goal.create({ data: { userId: A, title: "Goal A", type: "LEARNING" } });
    goalB = await prisma.goal.create({ data: { userId: B, title: "Goal B", type: "LEARNING" } });
    ...
    taskA = await prisma.task.create({ data: { userId: A, stageId: stageA.id, title: "Task A" } });
  ...
  it("A cannot complete B task", async () => { expect((await patchTask(request(A, `/api/tasks/${taskB.id}`, { method: "PATCH", body: JSON.stringify({ status: "COMPLETED" }) }), context(taskB.id))).status).toBe(404); expect((await prisma.task.findUnique({ where: { id: taskB.id } }))?.status).toBe("TODO"); });
  ...
  it("A AI cannot complete B task", async () => { const response = await aiCommand(request(A, "/api/ai/command", { method: "POST", body: JSON.stringify({ text: "selesaikan task", confirmed: true, context: { taskId: taskB.id } }) })); expect(response.status).not.toBe(200); expect((await prisma.task.findUnique({ where: { id: taskB.id } }))?.status).toBe("TODO"); });
